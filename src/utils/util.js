...
export default{
  ...
  formatDict:{
     ...
     */
    getDictList(dictTypeId){
      //添加业务字典值到缓存
      let list = [];
      if( sessionStorage.getItem(process.env.VUE_APP_NAME+'_dict_'+dictTypeId) == null ){
        if( dictTypeId != '' ) {
          let para1 = {dictTypeId: dictTypeId};
          getDict(para1).then(response => {
            list = response;
            sessionStorage.setItem(process.env.VUE_APP_NAME+'_dict_'+dictTypeId, JSON.stringify(list));
          })
        }
      }else{
        list = JSON.parse(sessionStorage.getItem(process.env.VUE_APP_NAME+'_dict_'+dictTypeId));  
      }
      return list
    },
    formatTableField(row, column, columnName, dictTypeId){
      if (column.property == columnName) {
        var list = this.getDictList(dictTypeId);
        for(var i = 0; i < list.length; i++){
          var item = list[i];
          if ( (row[columnName] + '') == item.dictId) {
  ...
  permissionValid:{
    ...
    hasPermissions: function(obj){
      var permissions = sessionStorage.getItem(process.env.VUE_APP_NAME+"_user_permission");
      var result = permissions.indexOf(obj);
      if( result == -1 ){
        return false;
      }else{
        return true;
