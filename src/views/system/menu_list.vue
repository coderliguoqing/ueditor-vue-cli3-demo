<template>
  <div class="menuTree">
    <el-button size="small" type="primary" icon="el-icon-refresh" @click="getMenuTreeList">
      重新加载
    </el-button>
    <el-button size="small" type="primary" icon="el-icon-plus" style="margin-bottom:15px;" @click="handleAdd">
      新建顶级菜单
    </el-button>
    <el-table
      :data="data"
      ...
      <el-table-column label="操作"  width="350">
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.children.length == 0" type="danger" @click="handleDelete(scope.$index, scope.row)">
            删除
          </el-button>
          <el-button type="primary" @click="handleShow(scope.$index, scope.row)">
            详情
          </el-button>
...
<script>
  import DataTransfer from '@/utils/datatranslate'
  import { getMenuTreeList, deleteMenu, editMenu } from '@/api/system/menu'
    ...
    computed: {
      ...
      data: function () {
        if (this.treeStructure) {
          let data = DataTransfer.treeToArray(this.dataSource, null, null, false)
          return data
        }
...
<style rel="stylesheet/scss" lang="scss" scoped>
  .menuTree{ margin: 15px 15px 15px 15px;}
  .ms-tree-space{
    position: relative;
    top: 1px;
